...
/**
 * Get subscription count by status (admin only)
 */
export async function getSubscriptionStats(): Promise<{
  data: { total: number; subscribed: number; unsubscribed: number } | null
  error?: string
}> {
  try {
    const supabase = await createServerClient()
    ...
    const subscribed = data.filter((s) => s.is_subscribed).length
    const unsubscribed = total - subscribed

    return { data: { total, subscribed, unsubscribed } }
  } catch (error) {
