    ...
    <div className="bg-white rounded-xl border-2 border-gray-200 shadow-sm overflow-hidden">
      ...
      <div className="p-3 md:p-4 space-y-2 md:space-y-3">
        ...
        <div className="flex flex-wrap gap-1.5">
          <span className={`px-2 py-0.5 text-[10px] md:text-xs font-semibold rounded-full border ${statusColors[project.status || 'active']}`}>
            {t(`status.${project.status}` as any)}
          </span>
          {project.is_long_term === true && (
            <span className="px-2 py-0.5 text-[10px] md:text-xs font-semibold rounded-full bg-purple-100 text-purple-800 border border-purple-200">
              {t('longTerm')}
            </span>
          )}
        </div>
